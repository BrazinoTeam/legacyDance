//
//  BuildARaftView.swift

import Foundation
import UIKit
import SnapKit

class BuildARaftView: UIView {
    ...
    private (set) var contentLabel: UILabel = {
        let label = UILabel.createLabel(withText: "   Zheng gathered branches and vines to build a raft. It took some time, but he managed to create a sturdy vessel. He successfully crossed the river and continued his journey.", font: .customFont(font: .ubuntuMono, style: .regular, size: 16), textColor: .white, paragraphSpacing: 1, lineHeightMultiple: 1.4, kern: 0.64)
        return label
    }()
    
    private (set) var btnGo: UIButton = {
        let btn = UIButton()
        btn.configureButton(withTitle: "Continue", font: .customFont(font: .mac, style: .regular, size: 20), titleColor: .cDarkRed, normalImage: .imgBtnSelect, highlightedImage: .imgBtnSelect)
