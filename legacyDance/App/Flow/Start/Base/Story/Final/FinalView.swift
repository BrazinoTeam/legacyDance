...
class FinalView: UIView {
    ...
    private (set) var btnGo: UIButton = {
        let btn = UIButton()
        btn.configureButton(withTitle: "Finish the story", font: .customFont(font: .mac, style: .regular, size: 20), titleColor: .cDarkRed, normalImage: .imgBtnSelect, highlightedImage: nil)
    ...
    private func setupConstraints() {
        ...
        labelContainer.snp.makeConstraints { make in
            make.left.right.equalToSuperview().inset(16)
            labelContainerBottomConstraint = make.bottom.equalTo(btnGo.snp.top).offset(-16.autoSize).constraint
